function [hE,hV]=wgPlot_animals(adjMat,coord,varargin)
...
plotParm={'markerSize',16,'lineWidth',0.1,'marker','.','MarkerEdgeColor',[1,0,0]};
siz=size(adjMat);
vrtxSiz=100;
edgeMap=cool;
vrtxMap=summer;
...
qq=unique([ii,jj]);
% minEWt=min(eWt);
minEWt= 0; %floor(min(eWt)*2)/2;  % Hilmi modified
maxEWt=max(eWt);
...
if vWeighted
  ...
  if exist('vrtxCol','var')
    ...
    hV=scatter(coord(:,1),coord(:,2),vWt,'filled','MarkerFaceColor',[1,0,0]);
  end
